...
def j_item(key=""):
    ...
    ).group_by(拾得物管理状況.拾得物ID).subquery()
    
    tmp = db.session.query(
        拾得物管理状況, 拾得物, 拾得物分類, ユーザ, 所属
    ).filter(
...
def d_item(id):
    data = db.session.query(拾得物管理状況, ユーザ).filter(
        拾得物管理状況.ユーザID == ユーザ.ID,
        拾得物管理状況.拾得物ID == id
    )
    list = []
    ...
    i, j = db.session.query(拾得物, 拾得物分類).filter(
        拾得物.拾得物分類ID == 拾得物分類.ID,
        拾得物.ID == id
    ).first()
    if i.画像 is None:
...
def req_item():
    if request.method == "POST":
        d = {
            ...
            "捜索物品名": request.form.get("item_name"),
            "捜索場所": request.form.get("place"),
            "色": request.form.get("color"),
            "特徴": request.form.get("spec")
        }
        req_item_reg(d)
        return redirect("/req_item")
    return render_template("req_item.html", j=j_user())  # ユーザ一覧を渡す
...
def req_list():
    data = db.session.query(遺失物捜索依頼, ユーザ).outerjoin(
    ユーザ, 遺失物捜索依頼.ユーザID == ユーザ.ID
    ).all()
    ...
    for i, u in data:
        obj.append({
            ...
            "色": i.色,
            "特徴": i.特徴,
            "登録日時": i.登録日時.strftime('%Y-%m-%d %H:%M') if isinstance(i.登録日時, datetime) else i.登録日時,
            "氏名": u.氏名 if u else "不明",
        })

    return render_template("req_list.html", items=obj)
